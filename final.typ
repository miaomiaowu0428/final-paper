...
=== JavaScript
#Text[JavaScript是一种轻量级的解释型语言，由Brendan Eich于1995为浏览器开发，作为Web核心技术之一，为Web前端页面提供灵动性。
...
== 自动配置HTTPS证书功能
#Text[在Web开发中，配置HTTPS证书一直是一项繁琐的工作，并且一次配置证书是不够的，HTTPS证书的有效期一般仅为数月，最长不过一年，所以在初次申请后还需要定期向证书颁发机构申请重新续签证书。在很多中小型团队并没有专门的运维岗位，大部分运维工作都由开发人员兼职完成，繁琐的证书配置与管理流程极大的加重了这部分开发人员的运维工作压力。其次，国内如阿里云等云服务厂商提供的HTTPS证书申请服务收费及其高昂，价格动辄达到数千元/年，对中小型团队以及个人开发者来说是一笔非常大的经济负担。如@fig:Alipay 所示#figure(image("media/阿里云收费.png"),caption: [阿里云配置证书收费页面],kind:image)<Alipay> 
...
所以Zino-cli的目标是帮助开发者向此类公益机构申请免费的HTTPS证书，并提供全自动的证书配置与管理功能，以及全自动定期完成续签。一方面为中小团队与个人开发者节约从云服务商渠道购买HTTPS证书带来的高昂成本，另一方面解放开发人员的生产力，使开发人员能从运维工作中抽身，专注于项目的功能与业务开发。]

== 本章小结
#Text[本章介绍了项目的功能，确定了涵盖从模板创建项目、使用模板原地初始化项目、提供可视化配置管理，以及自动集成与自动部署、自动配置HTTPS证书等在内的核心功能。满足了开发者们快速启动项目、快速管理项目配置、全自动的集成部署以及全自动的免费HTTPS证书配置与管理功能。解决了开发过程中的数个痛点，尤其是对于中小团队及个人开发者来说，本项目能大幅降低他们的开发的成本，提高开发效率。]
...
#Text[本工具将提供init命令用于将当前目录按照模版初始化为一个项目，默认使用目录名作为项目名。接受一个\--project-name选项用做项目名称。开启选项后可以填入额外参数用于指定项目名称。接受一个\--template-path选项，开启选项后可填入额外参数用于指定模板仓库。若不开启选项则使用预先制作好的默认仓库。具体流程与创建新项目类似，此处不另附图。]
...
用户访问配置服务页面时，前端在页面元素渲染完成后，向后端发起请求获取当前项目的配置信息，包括项目名称，项目路径与配置文件内容，并将已开启的配置选项情况渲染在页面中。] 
...
=== 配置HTTPS证书
首先从加载好的配置文件的文本中解析出需要配置证书的域名，以及证书管理相关事项的联系人邮箱信息。然后建立请求转发程序，用以监听配置文件中指定的端口（一般HTTPS请求使用443端口），然后在此端口收到的TLS流中解包出实际的HTTP请求，并将其转发给用户程序端口（同样在配置文件中指定，默认为6080端口）。收到用户程序的响应后将其打包进tls流返回给请求者。接下来将域名、邮箱等信息提交给Let's Encrypt的验证服务，Let's Encrypt会向指定域名发起HTTPS请求以验证指定域名确实存在服务。转发程序按上述流程转发处理Let's Encrypt发起的HTTPS请求。验证通过后将收到的证书信息及缓存信息保存至配置文件中指定的缓存目录。如@fig:ACME 所示。


#figure(image("media/ACME.png"),caption: [证书管理流程])<ACME>


=== 部署流程
首先从加载好的配置文件的文本中解析出需要监控的远端仓库信息，包括仓库名，分支名等。然后启动用户程序。用户程序启动后开启轮询功能，定期检查远端仓库指定分支最新一次提交哈希值是否与本地对应分支的最新提交哈希值一致。若一致则说明远端无更新。无更新则结束轮询并等待配置文件中指定的轮询间隔。若远端最新提交哈希值与本地不一致则说明远端仓库有更新。有更新则拉取新代码，运行编译检查和测试。若检查和测试成功则杀灭旧用户程序进程，以新版本代码开启进程，实现替换。如@fig:CI-CD 所示。
== 本章小节
本章对项目的功能设计进行了大致的介绍，明确了开发目标。

#figure(image("media/CI-CD.png"),
  caption: [
    CI/CD
  ],kind:image
)<CI-CD>
